#!/usr/bin/env bash
# BEACON-production comparison launch for TRACE-GRPO on ScienceWorld.
#
# This script intentionally has no min/smoke/debug batch profile. It fixes
# task sampling, environment, rollout count, temperatures, evaluation cadence,
# and reward/metric semantics to BEACON's ScienceWorld production baseline.
# Parameters whose meaning changes under packed-trajectory training are adapted
# below and logged explicitly.
set -euo pipefail
...
# Match BEACON ScienceWorld launch environment where possible.  The H200
# production path uses the same vLLM/FA2 settings as the validated 7B smoke.
...
# BEACON ScienceWorld production task/eval knobs.
ENGINE=vllm
TRAIN_DATA_SIZE=16
VAL_DATA_SIZE=128
GROUP_SIZE=8
...
# (fsdp_workers.py:326). So to reproduce MiGPO's 1-update-per-batch cadence
...
# BEACON uses 512 response tokens per step. TRACE-GRPO packs the whole trajectory
# into one response axis, so data.max_response_length must cover the trajectory.
...
LOG_ROOT=${LOG_ROOT:-$REPO_ROOT/outputs/sciworld_beacon_prod}
...
TEXT_FEEDBACK_SHOW_SCORE=${TEXT_FEEDBACK_SHOW_SCORE:-1}
...
log "[beacon-prod] script=run_sciworld_trace_l3_beacon_prod_llmjudge.sh"
...
log "[beacon-prod] beacon_semantics: use_invalid_action_penalty=true coef=0.1 (matches MiGPO sciworld baseline run_sciworld.sh:70-71)"
log "[beacon-prod] packed_adapted: ppo_mini=$PPO_MINI_BATCH_SIZE prompts (verl scales ×rollout.n=8 → 128 rows = 1 update/batch, matches MiGPO) ppo_micro=$PPO_MICRO_BATCH_SIZE_PER_GPU logprob_micro=$LOG_PROB_MICRO_BATCH_SIZE_PER_GPU"
...
log "[beacon-prod] train/val task pools: BEACON L0 train/test via use_beacon_train_pool=true,use_beacon_val_pool=true"
log "[beacon-prod] trainer.total_epochs=$STEPS because train dataloader has one BEACON batch per epoch"
...
python -m trace_grpo.launchers.main_ppo \
  ...
  +algorithm.trace_grpo.trace_l3_suppress_cap=0.3 \
  +algorithm.trace_grpo.trace_l3_boost_cap=0.1 \
  ...
  trainer.experiment_name=trace_l3_beacon_prod_${MODEL_TAG} \
  ...
  +ray_kwargs.ray_init.runtime_env.env_vars.TEXT_FEEDBACK_SHOW_SCORE="'$TEXT_FEEDBACK_SHOW_SCORE'" \
